...
def call(params = [:]) {
    ...
    def denyList = "--denylist-test ";

    if (params['skipSecureBoot']) {
        denyList += "*.uefi-secure"
    } else if (params['skipUEFI']) {
        denyList += "*.uefi*"
    }
    ...
    try {
        shwrap("cd ${cosaDir} && cosa kola testiso --inst-insecure ${extraArgs} ${denyList} --output-dir ${outputDir}/${id}")
    } finally {
